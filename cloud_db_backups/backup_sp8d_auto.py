...
def list_existing_backups():
    ...
    for i, file in enumerate(sp8d_files[:5]):  # 只显示最新的5个
...
if __name__ == "__main__":
    print("🔄 云端sp8d数据库自动备份工具")
    print("=" * 50)
    
    # 检查pg_dump是否可用
    try:
        result = subprocess.run(["pg_dump", "--version"], capture_output=True, check=True)
        print(f"📦 PostgreSQL 客户端版本: {result.stdout.decode().strip()}")
    ...
    print(f"\n🔄 正在自动创建新的sp8d数据库备份...")
    print(f"注意: 这只会备份数据到本地，不会修改云端数据库")
