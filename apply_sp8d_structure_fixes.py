...
"""
直接应用SP8D数据库结构修复
使用SQLAlchemy直接执行迁移SQL，遵循CLAUDE.md规范
"""
...
logger = logging.getLogger('SP8D_Structure_Fix')
...
class SP8DStructureFix:
    ...
    def backup_sp8d(self):
        ...
        backup_dir = os.path.join(os.getcwd(), 'cloud_db_backups')
        os.makedirs(backup_dir, exist_ok=True)
        
        backup_file = f'sp8d_structure_fix_backup_{self.timestamp}.sql'
    ...
    def run(self):
        ...
        try:
            ...
            # 3. 应用修复
            self.fix_approval_step_fields(conn)
            self.fix_performance_amount_types(conn) 
            self.fix_performance_targets_constraints(conn)
            ...
            if self.verify_fixes(conn):
                ...
                logger.info("✅ 审批分支功能字段已添加")
                logger.info("✅ 金额字段类型已统一为NUMERIC(15,2)")
                logger.info("✅ 约束和默认值已统一")
