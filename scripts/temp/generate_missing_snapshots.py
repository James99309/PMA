...
为所有 code_definition_snapshot 为 NULL 的产品生成快照数据。
...
def generate_missing_snapshots(dry_run=False):
    """
    批量生成缺失的产品快照
        ...
        if total_count > 0:
            ...
            for i, product in enumerate(products_without_snapshot[:10], 1):
                print(f"  {i}. ID={product.id}, MN={product.product_mn}, "
                      f"名称={product.product_name or product.subcategory.name if product.subcategory else 'N/A'}")
            if total_count > 10:
...
def main():
    ...
    try:
        generate_missing_snapshots(dry_run=args.dry_run)
