...
# -*- coding: utf-8 -*-
"""
修复产品快照数据：添加 use_in_code 字段并过滤非编码规格

功能：
1. 遍历所有产品的 code_definition_snapshot
2. 查询每个规格字段的 use_in_code 值
3. 移除 use_in_code=False 的非编码规格
4. 为所有规格添加 use_in_code 字段

使用方法：
    python3 scripts/temp/fix_product_snapshots_use_in_code.py
...
# 创建最小化的Flask应用（避免导入所有路由和依赖）
app = Flask(__name__)
app.config.from_object(Config)
db.init_app(app)


def fix_product_snapshots():
    """修复所有产品的快照数据"""
    with app.app_context():
        ...
        if fixed_count > 0:
            ...
            db.session.commit()
            print(f'✅ 成功修复 {fixed_count} 个产品的快照数据')
        else:
            print('\n✅ 所有产品快照数据已经正确，无需修复')
