...
"""
应用SP8D数据库实际存在的结构修复
根据实际检查结果只修复需要的部分，遵循CLAUDE.md规范
"""
...
logger = logging.getLogger('SP8D_Actual_Fix')
...
class SP8DActualFix:
    ...
    def fix_approval_step_fields(self, conn):
        ...
        logger.info("🔧 检查并修复approval_step表审批分支功能字段...")
        
        if not self.check_table_exists(conn, 'approval_step'):
            logger.error("❌ approval_step表不存在")
            return 0
        ...
        if added_count > 0:
            ...
            for idx_name, column in indexes:
                try:
                    ...
                    logger.info(f"  ⚠️ 索引创建可能失败: {idx_name} - {e}")
    ...
    def fix_performance_amount_types(self, conn):
        ...
        for table_name, fields in tables_to_check:
            if not self.check_table_exists(conn, table_name):
                logger.info(f"  ⏭️ 表不存在: {table_name}")
                continue
            
            for field_name, comment in fields:
                current_type = self.check_column_type(conn, table_name, field_name)
                if current_type == 'double precision':
                    sql = f"ALTER TABLE {table_name} ALTER COLUMN {field_name} TYPE NUMERIC(15,2)"
                    cursor.execute(sql)
                    logger.info(f"  ✅ 转换类型: {table_name}.{field_name} -> NUMERIC(15,2)")
                    converted_count += 1
                elif current_type:
                    logger.info(f"  ⏭️ 类型正确: {table_name}.{field_name} ({current_type})")
                else:
                    logger.info(f"  ⏭️ 字段不存在: {table_name}.{field_name}")
    ...
    def fix_performance_targets_constraints(self, conn):
        ...
        if not self.check_table_exists(conn, 'performance_targets'):
            logger.info("  ⏭️ 表不存在: performance_targets")
            return 0
        ...
        # 检查created_by约束
        if self.check_column_exists(conn, 'performance_targets', 'created_by'):
            ...
            logger.info("  ⏭️ 字段不存在: performance_targets.created_by")
        
        # 设置rate字段默认值
        rate_fields = ['customers_rate', 'implant_rate', 'projects_rate', 'sales_rate']
        
        for field in rate_fields:
            if self.check_column_exists(conn, 'performance_targets', field):
                ...
                logger.info(f"  ⏭️ 字段不存在: performance_targets.{field}")
    ...
    def verify_fixes(self, conn):
        ...
        # 验证approval_step字段
        if self.check_table_exists(conn, 'approval_step'):
            cursor.execute("""
                SELECT column_name 
                FROM information_schema.columns 
            ...
            if missing_fields:
                logger.warning(f"  ⚠️ 部分字段未添加: {missing_fields}")
            else:
                logger.info(f"  ✅ approval_step分支字段完整: {len(approval_fields)}/8")
        
        # 验证金额字段类型
        performance_tables = ['performance_statistics', 'performance_targets']
        for table_name in performance_tables:
            if self.check_table_exists(conn, table_name):
                cursor.execute("""
                    SELECT column_name, data_type
                    FROM information_schema.columns 
                    WHERE table_name = %s AND column_name LIKE '%amount%'
                """, (table_name,))
                
                amount_fields = cursor.fetchall()
                for field in amount_fields:
                    if field['data_type'] != 'numeric':
                        logger.warning(f"  ⚠️ 类型未转换: {table_name}.{field['column_name']} = {field['data_type']}")
                    else:
                        logger.info(f"  ✅ 类型正确: {table_name}.{field['column_name']} = numeric")
        
        # 验证约束（如果表存在）
        if self.check_table_exists(conn, 'performance_targets') and self.check_column_exists(conn, 'performance_targets', 'created_by'):
            cursor.execute("""
                SELECT is_nullable, column_default
                FROM information_schema.columns 
            ...
            if created_by_info['is_nullable'] == 'YES':
                logger.warning("  ⚠️ 约束未设置: performance_targets.created_by 仍可为空")
            else:
    ...
    def run(self):
        ...
        try:
            ...
            if added_fields > 0 or converted_types > 0 or fixed_constraints > 0:
                logger.info("✅ SP8D数据库结构差异已修复，与本地数据库更加一致")
            else:
                logger.info("ℹ️ SP8D数据库结构已是最新状态，无需修复")
