...
class Users(Base):
    ...
    projects: Mapped[List['Projects']] = relationship('Projects', foreign_keys='[Projects.locked_by]', back_populates='users')
    projects_: Mapped[List['Projects']] = relationship('Projects', foreign_keys='[Projects.owner_id]', back_populates='owner')
...
class ApprovalProcessTemplate(Base):
    ...
    required_fields: Mapped[Optional[dict]] = mapped_column(JSON, server_default=text("'[]'::jsonb"), comment='发起审批时必填字段列表')
...
class Projects(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['locked_by'], ['users.id'], name='projects_locked_by_fkey'),
    ...
    owner_id: Mapped[Optional[int]] = mapped_column(Integer)
    locked_reason: Mapped[Optional[str]] = mapped_column(String(100))
    locked_by: Mapped[Optional[int]] = mapped_column(Integer)
    locked_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime)
    last_activity_date: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('now()'))
    activity_reason: Mapped[Optional[str]] = mapped_column(String(50))

    users: Mapped[Optional['Users']] = relationship('Users', foreign_keys=[locked_by], back_populates='projects')
    owner: Mapped[Optional['Users']] = relationship('Users', foreign_keys=[owner_id], back_populates='projects_')
...
class ApprovalRecord(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['approver_id'], ['users.id'], name='fk_approval_record_approver_id'),
        ForeignKeyConstraint(['instance_id'], ['approval_instance.id'], name='approval_record_instance_id_fkey'),
        ForeignKeyConstraint(['step_id'], ['approval_step.id'], name='fk_approval_record_step_id'),
        PrimaryKeyConstraint('id', name='approval_record_temp_pkey')
    )
