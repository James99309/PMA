...
def quotations_list_ajax():
    ...
    try:
        # 添加调试日志
        current_app.logger.info("开始AJAX请求处理")
        
        # 简单测试：先返回基础数据
        return jsonify({
            'success': True,
            'html': '<tr><td colspan="8" class="text-center">AJAX测试成功</td></tr>',
            'total_count': 0,
            'loaded_count': 0,
            'has_more': False,
            'statistics': {
        ...
        current_app.logger.error(f"AJAX调试时出错: {str(e)}", exc_info=True)
        return jsonify({
            'success': False,
            'message': f'调试失败: {str(e)}',
            'html': '<tr><td colspan="8" class="text-center text-muted">调试模式 - 请检查日志</td></tr>',
            'total_count': 0,
            'loaded_count': 0,
            'has_more': False,
            'statistics': {
...
def create_quotation():
    ...
    pass  # 暂时空实现，先确保AJAX函数能正常工作
        ...
        # 审批状态筛选
        if approval_status:
            query = query.filter(Quotation.approval_status == approval_status)
        ...
        # 计算统计数据 - 基于筛选条件重新构建查询来计算统计
        # 重新构建基础查询用于统计计算
        stats_query = get_viewable_data(Quotation, current_user)
        stats_project_joined = False
        
        # 应用相同的搜索和筛选条件
        if search:
            if not stats_project_joined:
                stats_query = stats_query.join(Project, Quotation.project_id == Project.id)
                stats_project_joined = True
        ...
        if project_type_filter:
            ...
            if project_type_filter == 'channel_follow':
                stats_query = stats_query.filter(Project.project_type.in_(['channel_follow', '渠道跟进']))
            elif project_type_filter == 'sales_focus':
                stats_query = stats_query.filter(Project.project_type.in_(['sales_focus', '销售重点']))
            elif project_type_filter == 'marketing_focus':
                stats_query = stats_query.filter(Project.project_type.in_(['sales_focus', 'channel_follow', '销售重点', '渠道跟进']))
            else:
        ...
        if approval_status:
            stats_query = stats_query.filter(Quotation.approval_status == approval_status)
        
        # 基于筛选后的数据计算统计 - 使用数据库查询而不是内存计算提高性能
        total_amount_query = stats_query.with_entities(func.sum(Quotation.amount)).scalar()
        total_stats_amount = round((total_amount_query or 0) / 10000, 2)
        ...
        statistics = {
            ...
            'approved': approved_stats.count(),
            'approved_amount': approved_amount,
            'pending': pending_stats.count(),
            'pending_amount': pending_amount,
            'draft': draft_stats.count(),
        ...
        current_app.logger.error(f"AJAX筛选报价单列表时出错: {str(e)}", exc_info=True)
        
        # 错误时返回基于筛选条件的统计数据，避免前端显示异常
        try:
            # 重新构建查询用于错误时的统计计算
            error_stats_query = get_viewable_data(Quotation, current_user)
            error_project_joined = False
            
            # 尝试应用筛选条件（如果可能的话）
            try:
                if search:
                    if not error_project_joined:
                        error_stats_query = error_stats_query.join(Project, Quotation.project_id == Project.id)
                        error_project_joined = True
                    error_stats_query = error_stats_query.filter(
                        or_(
                            Quotation.quotation_number.ilike(f'%{search}%'),
                            Project.project_name.ilike(f'%{search}%')
                        )
                ...
                if owner_filter:
                    error_stats_query = error_stats_query.filter(Quotation.owner_id == owner_filter)
                if project_type_filter:
                    if not error_project_joined:
                        error_stats_query = error_stats_query.join(Project, Quotation.project_id == Project.id)
                        error_project_joined = True
                    ...
                    if project_type_filter == 'channel_follow':
                        error_stats_query = error_stats_query.filter(Project.project_type.in_(['channel_follow', '渠道跟进']))
                    elif project_type_filter == 'sales_focus':
                        error_stats_query = error_stats_query.filter(Project.project_type.in_(['sales_focus', '销售重点']))
                    elif project_type_filter == 'marketing_focus':
                        error_stats_query = error_stats_query.filter(Project.project_type.in_(['sales_focus', 'channel_follow', '销售重点', '渠道跟进']))
                    else:
                        error_stats_query = error_stats_query.filter(Project.project_type == project_type_filter)
                if project_stage_filter:
                    if not error_project_joined:
                        error_stats_query = error_stats_query.join(Project, Quotation.project_id == Project.id)
                        error_project_joined = True
                    error_stats_query = error_stats_query.filter(Project.current_stage == project_stage_filter)
                if approval_status:
                    error_stats_query = error_stats_query.filter(Quotation.approval_status == approval_status)
            except:
                pass  # 如果筛选条件有问题，使用基础查询
            
            # 计算错误时的统计数据
            error_total_amount_query = error_stats_query.with_entities(func.sum(Quotation.amount)).scalar()
            error_total_amount = round((error_total_amount_query or 0) / 10000, 2)
            
            error_approved_stats = error_stats_query.filter(Quotation.approval_status == 'approved')
            error_approved_amount_query = error_approved_stats.with_entities(func.sum(Quotation.amount)).scalar()
            error_approved_amount = round((error_approved_amount_query or 0) / 10000, 2)
            
            error_pending_stats = error_stats_query.filter(Quotation.approval_status.in_(['pending', 'in_progress']))
            error_pending_amount_query = error_pending_stats.with_entities(func.sum(Quotation.amount)).scalar()
            error_pending_amount = round((error_pending_amount_query or 0) / 10000, 2)
            
            error_draft_stats = error_stats_query.filter(Quotation.approval_status == 'draft')
            error_draft_amount_query = error_draft_stats.with_entities(func.sum(Quotation.amount)).scalar()
            error_draft_amount = round((error_draft_amount_query or 0) / 10000, 2)
            
            error_statistics = {
                'total': error_stats_query.count(),
                'total_amount': error_total_amount,
                'approved': error_approved_stats.count(),
                'approved_amount': error_approved_amount,
                'pending': error_pending_stats.count(),
                'pending_amount': error_pending_amount,
                'draft': error_draft_stats.count(),
                'draft_amount': error_draft_amount
            }
        except:
            # 如果连统计数据都无法获取，则返回空统计
            error_statistics = {
