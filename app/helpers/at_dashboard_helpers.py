...
# KPI 实际采集已收口到后端服务,此处 re-export 保持既有引用不破(单向依赖 helper→service)
from app.services.kpi_actual_service import (  # noqa: F401
    _KPI_ACTUAL_FNS, _sum_money, _conv_money,
    _qualified_customer_filters, _qualified_project_filters,
    _act_hr_task_count,
)
...
def _kpi_score_summary(user):
    ...
    if not codes:
        return None
    _ALIAS = {'sales_target': 'sales_amount'}   # item_code → 看板/采集 code(仅 sales 不同,余恒等)
    # 权重:个人覆盖 > 角色覆盖 > 方案默认(键统一到看板 code,与 codes/采集器对齐;
    # 否则 sales_target 权重挂不上 sales_amount → sales 项被漏掉,得分虚高)
    wmap = {}
    sch = get_role_scheme(user.role) if user.role else None
    for it in (sch or []):
        wmap[_ALIAS.get(it['item_code'], it['item_code'])] = float(it.get('weight') or 0)
    for t in RolePerformanceTarget.query.filter_by(role_code=user.role, year=year).all():
        if getattr(t, 'weight', None) is not None:
            wmap[_ALIAS.get(t.item_code, t.item_code)] = float(t.weight)
    for t in UserPerformanceTarget.query.filter_by(user_id=user.id, year=year).all():
        if getattr(t, 'weight_override', None) is not None:
            wmap[_ALIAS.get(t.item_code, t.item_code)] = float(t.weight_override)
    try:
