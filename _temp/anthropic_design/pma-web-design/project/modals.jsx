...
function CreateShipmentModal({ open, order, onClose }) {
  ...
  const items = order?.items || [
    { desc: "常规数字基站", model: "Mark3000BS", arrived: 0, shipped: 0, avail: 1 },
    { desc: "多信道分合路器", model: "FHJ400-2", arrived: 0, shipped: 0, avail: 1 },
    { desc: "数字智能信道机", model: "Mark1000 MAX", arrived: 0, shipped: 0, avail: 1 },
  ].map(i => ({ ...i, arrived: i.qty || 0, shipped: 0, avail: i.qty || 1 }));
  ...
  return (
    <ModalShell
      ...
      footer={<>
        <Btn variant="ghost" onClick={onClose}>取消</Btn>
        <Btn variant="accent" iconL="truck">创建发货单</Btn>
      </>}
    >
      {/* Destination toggle */}
      <Field label="发货去向">
        <div style={{ display: "flex", gap: 8 }}>
          {[
            { v: "customer",  label: "发给客户订单", icon: "users" },
            { v: "warehouse", label: "入公司仓库",   icon: "archive" },
          ].map(opt => {
            ...
            return (
              <button key={opt.v} onClick={() => setTarget(opt.v)}
                style={{
                  ...
                  background: active ? "var(--accent-tint)" : "var(--bg-page)",
                ...
                <span style={{ fontSize: 13.5, fontWeight: 500, color: "var(--ink)" }}>{opt.label}</span>
              </button>
            );
          })}
        </div>
      ...
      {target === "customer" && (
        <Field label="目标客户订单" required>
          <Select
            value="" onChange={() => {}}
            options={["选择客户订单…", "SO202605-001 · 上海大展通信", "SO202605-002 · 上海大展通信", "SO202605-003 · 上海常森电子"]}
            style={{ width: "100%" }}
          />
        </Field>
      )}
      ...
      {/* Items */}
      <Field label="发货明细" hint={`${items.length} 项物料`}>
        <div style={{ border: "1px solid var(--line)", borderRadius: 8, overflow: "hidden" }}>
          <table style={{ width: "100%", fontSize: 12.5 }}>
            <thead>
              <tr style={{ background: "var(--bg-page)", borderBottom: "1px solid var(--line)" }}>
                {["产品", "型号", "到货", "已发", "可发", "本次发货"].map((h, i) => (
                  <th key={h} style={{
                    textAlign: i === 0 ? "left" : i >= 5 ? "right" : "center",
                    padding: "10px 14px", fontSize: 10.5, fontWeight: 500,
            ...
            <tbody>
              {items.flatMap((it, i) => {
                const rows = [];
                rows.push(
                  <tr key={`row-${i}`} style={{ borderBottom: skipSn[i] && i === items.length - 1 ? 0 : "1px solid var(--line)" }}>
                    <td style={{ padding: "12px 14px", fontWeight: 500 }}>{it.desc}</td>
                    <td style={{ padding: "12px 14px" }} className="mono">{it.model}</td>
                    <td style={{ padding: "12px 14px", textAlign: "center" }} className="mono tab-num dim">{it.arrived ?? 0}</td>
                    <td style={{ padding: "12px 14px", textAlign: "center" }} className="mono tab-num dim">{it.shipped ?? 0}</td>
                    <td className="mono tab-num" style={{ color: "var(--success)", padding: "12px 14px", textAlign: "center" }}>{it.avail || 1}</td>
                    <td style={{ padding: "12px 14px", textAlign: "right" }}>
                      <Stepper value={qty[i]} min={0} max={it.avail || 1}
                        onChange={v => setQty({ ...qty, [i]: v })} />
                    </td>
                  </tr>
                );
                if (!skipSn[i]) {
                  rows.push(
                    <tr key={`sn-${i}`} style={{ background: "var(--bg-page)", borderBottom: i === items.length - 1 ? 0 : "1px solid var(--line)" }}>
                      <td colSpan={6} style={{ padding: "10px 14px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                          <span className="dim mono" style={{ fontSize: 10.5, letterSpacing: "0.08em" }}>SN</span>
                          <span className="dim" style={{ fontSize: 11.5 }}>前缀</span>
                          <input value={snPrefix[i]} onChange={e => setSnPrefix({ ...snPrefix, [i]: e.target.value })}
                            className="mono" style={{
                              width: 140, padding: "5px 8px", border: "1px solid var(--line-2)",
                              borderRadius: 4, fontSize: 12, background: "var(--bg-elev)", outline: "none",
                            }}/>
                          <span className="dim" style={{ fontSize: 11.5 }}>起始号</span>
                          <input value={snStart[i]} onChange={e => setSnStart({ ...snStart, [i]: e.target.value })}
                            className="mono" style={{
                              width: 60, padding: "5px 8px", border: "1px solid var(--line-2)",
                              borderRadius: 4, fontSize: 12, background: "var(--bg-elev)", outline: "none",
                            }}/>
                          <span className="mono dim" style={{ fontSize: 11 }}>
                            → {snPrefix[i]}{snStart[i]} … {snPrefix[i]}{String(Number(snStart[i] || 0) + (qty[i] || 1) - 1).padStart((snStart[i] || "001").length, "0")}
                          </span>
                          <label style={{
                            marginLeft: "auto", display: "inline-flex", alignItems: "center", gap: 6,
                            fontSize: 12, color: "var(--ink-3)", cursor: "pointer",
                          }}>
                            <input type="checkbox" checked={!!skipSn[i]} onChange={e => setSkipSn({ ...skipSn, [i]: e.target.checked })} />
                            无序列号
                          </label>
                        </div>
                      </td>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <Field label="承运商">
          <Input placeholder="物流公司名称" style={{ width: "100%" }} />
        </Field>
        <Field label="运单号">
          <Input placeholder="物流单号" style={{ width: "100%" }} />
        </Field>
      </div>
    </ModalShell>
  );
