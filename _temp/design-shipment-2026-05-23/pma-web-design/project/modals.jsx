...
function CreateShipmentModal({ open, order, onClose }) {
  ...
  const validate = () => {
    const errs = {};
    if (target === "customer" && !customerOrder) errs.customerOrder = "请选择目标客户订单";
    if (totalQty === 0)                          errs.qty = "请至少填写一件本次发货数量";
    return errs;
  };
  ...
  const onSubmit = () => {
    ...
    if (Object.keys(errs).length > 0) {
      window.toast?.warn?.("无法创建发货单", Object.values(errs)[0]);
      return;
    }
    window.toast?.success?.("发货单已创建", `本次共发货 ${totalQty} 件`);
    onClose();
  };
  ...
  return (
    <ModalShell
      ...
      footer={<>
        <Btn variant="ghost" onClick={onClose}>取消</Btn>
        <Btn variant="accent" iconL="truck" onClick={onSubmit}>
    ...
    >
      ...
      {target === "customer" && (
        <Field label="目标客户订单" required>
          <Select
            value={customerOrder} onChange={(v) => { setCustomerOrder(v); setErrors(e => ({...e, customerOrder: undefined})); }}
            options={[
              { value: "", label: "选择客户订单…" },
            ...
            style={{ width: "100%", borderColor: errors.customerOrder ? "var(--danger)" : undefined }}
          />
          <FieldError>{errors.customerOrder}</FieldError>
        </Field>
      )}

      {/* Items */}
      <Field label="发货明细" hint={errors.qty ? <span style={{ color: "var(--danger)" }}>{errors.qty}</span> : `${items.length} 项物料 · 本次共 ${totalQty} 件`}>
...
function SignReceiptModal({ open, shipment, onClose, onSubmit }) {
  const [note, setNote] = useState("");
  ...
  return (
    <ModalShell
      ...
      footer={<>
        <Btn variant="ghost" onClick={onClose}>取消</Btn>
        <Btn variant="accent" iconL="check" onClick={onSubmit}>确认签收</Btn>
      </>}
    >
      <InfoStrip items={[
        { k: "发货单号", v: shipment?.id || "—", mono: true, strong: true },
        { k: "目标",     v: shipment?.target || "—" },
        { k: "数量",     v: (shipment?.qty ?? "—") + " 件", mono: true },
        { k: "承运商",   v: (shipment?.carrier || "—") + (shipment?.waybill ? ` · ${shipment.waybill}` : "") },
      ]} />
      <div style={{ height: 18 }} />
      <Field label="签收日期" required>
        <input
          type="date" value={signDate}
      ...
      <Field label="签收单" required hint="可拍照上传(JPG/PNG)或扫描 PDF">
        <DropZone name="签收单" />
      </Field>
      <Field label="备注">
        <textarea
...
Object.assign(window, {
  ...
  FilePreviewModal, fmtFileSize, SignReceiptModal,
});
