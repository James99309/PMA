...
function App() {
  ...
  // Build breadcrumbs
  const breadcrumbs =
    route.name === "list"     ? [{ label: "采购管理" }, { label: "采购订单" }] :
    route.name === "detail"   ? [
        { label: "采购管理", onClick: goBack },
        { label: "采购订单", onClick: goBack },
        { label: route.orderId },
      ] :
    route.name === "inventory"  ? [{ label: "库存管理" }, { label: "库存" }] :
    route.name === "globalflow" ? [
        { label: "库存管理", onClick: () => setRoute({ name: "inventory" }) },
        { label: "库存",     onClick: () => setRoute({ name: "inventory" }) },
        { label: "全局流水" },
      ] :
    [];
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "var(--bg-page)" }}>
      ...
      <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
        ...
        <main style={{ flex: 1, minHeight: 0 }}>
          {route.name === "list" ? (
            ...
            <InventoryScreen
              onOpenGlobal={() => setRoute({ name: "globalflow" })}
              onOpenAdjust={(r) => setAdjustRow(r)}
              onOpenImport={() => openModal("excel-import")}
            />
          ) : route.name === "globalflow" ? (
            <GlobalFlowScreen onBack={() => setRoute({ name: "inventory" })} />
          ) : (
            <DetailScreen
      ...
      <AdjustStockModal     open={!!adjustRow} row={adjustRow}
        companyName="和源通信(上海)股份有限公司"
        onClose={() => setAdjustRow(null)} />
      <ExcelImportModal     open={modal === "excel-import"} onClose={closeModal} />
      ...
      <TweaksPanel>
        ...
        <TweakSection label="库存模块" />
        <TweakButton onClick={() => setRoute({ name: "inventory" })}>库存管理 · 主页</TweakButton>
        <TweakButton onClick={() => setRoute({ name: "globalflow" })}>库存管理 · 全局流水</TweakButton>
        <TweakButton onClick={() => setAdjustRow(window.PMA_INV.STOCK[0])}>弹窗 · 调整库存</TweakButton>
        <TweakButton onClick={() => setModal("excel-import")}>弹窗 · Excel 导入</TweakButton>
      </TweaksPanel>
    </div>
  );
}
