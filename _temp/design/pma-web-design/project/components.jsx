...
const NAV = [
  ...
  { key: "purchase",   icon: "cart",    label: "采购管理",
    sub: [
      { key: "po",       label: "采购订单", route: "list" },
  ...
  { key: "inventory",  icon: "box",     label: "库存管理",
    sub: [
      { key: "stock",   label: "库存",       route: "inventory" },
      { key: "flow",    label: "全局流水",   route: "globalflow" },
    ]
  },
...
function Sidebar({ collapsed, onToggle, currentRoute, onNavigate }) {
  const [hovered, setHovered] = useState(false);
  const showExpanded = !collapsed || hovered;
  const isFloating = collapsed && hovered;

  const isInPurchase  = currentRoute === "list" || currentRoute === "detail";
  const isInInventory = currentRoute === "inventory" || currentRoute === "globalflow";
  ...
  return (
    ...
    >
      ...
      <nav style={{ flex: 1, overflowY: "auto", padding: "12px 8px", display: "flex", flexDirection: "column", gap: 2 }}>
        {NAV.map(item => {
          const isActive = (item.key === "purchase" && isInPurchase) || (item.key === "inventory" && isInInventory);
          return (
          <div key={item.key}>
            <button
              title={!showExpanded ? item.label : undefined}
              onClick={() => {
                if (item.key === "purchase")  onNavigate?.("list");
                if (item.key === "inventory") onNavigate?.("inventory");
              }}
              style={{
            ...
            {showExpanded && isActive && item.sub && (
              <div style={{ marginLeft: 30, marginTop: 2, marginBottom: 6, display: "flex", flexDirection: "column", gap: 1, borderLeft: "1px solid var(--line)", paddingLeft: 10 }}>
                {item.sub.map(s => {
                  const isCurrent = s.route && s.route === currentRoute;
                  return (
                    <button key={s.key}
