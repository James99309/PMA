...
function DetailScreen({ orderId, onBack, onOpenModal, timelinePos }) {
  ...
  const [signing, setSigning] = useState(null); // shipment object
  const openPreview = (stageKey, stageLabel, files) => setPreview({ stageKey, stageLabel, files });
  const closePreview = () => setPreview(null);
  const previewShipmentFile = (shipment, kind) => {
    const file = kind === "express" ? shipment.express : shipment.receipt;
    const label = kind === "express" ? "快递单" : "签收单";
    if (file) setPreview({ stageLabel: `${shipment.id} · ${label}`, files: [file] });
  };
  ...
  return (
    <div style={{ padding: "32px 40px 80px", maxWidth: 1400, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <Section title="发货记录" subtitle={
            ...
            <ShipmentList
              ...
              canCreate={order.stage >= 4}
              onCreate={() => onOpenModal("ship")}
              onPreview={previewShipmentFile}
              onSign={(shipment) => setSigning(shipment)}
            />
          </Section>
        </div>
      ...
      <SignReceiptModal
        ...
        onSubmit={() => {
          window.toast?.success?.("已签收入库", `${signing?.id} 标记为已签收`);
          setSigning(null);
        }}
      />
    </div>
...
function ShipmentList({ shipments, canCreate, onCreate, onPreview, onSign }) {
...
function ShipmentRow({ shipment, isExpanded, onToggle, isLast, onPreview, onSign }) {
  const cellBorder = isExpanded ? "0" : (isLast ? "0" : "1px solid var(--line-soft)");
  const topBorder = "1px solid var(--line-soft)";
  const isSigned = shipment.status === "已签收";
  const statusTone = isSigned ? "success" : "info";
  return (
    <>
      <tr
        onClick={onToggle}
      ...
      >
        ...
        <td style={{ padding: "14px 20px", borderTop: topBorder }}>
          <Pill tone={statusTone} dot>{shipment.status}</Pill>
        </td>
        <td style={{ padding: "14px 20px", borderTop: topBorder }} className="mono tab-num dim">
          {shipment.date}
        </td>
        <td style={{ padding: "14px 20px", borderTop: topBorder, textAlign: "right" }}>
          {!isSigned && (
            <button onClick={(e) => { e.stopPropagation(); onSign?.(shipment); }}
              style={{
      ...
      {isExpanded && (
        <tr style={{ background: "var(--bg-page)" }}>
          <td colSpan={7} style={{
            ...
            <ShipmentDetail shipment={shipment} onPreview={onPreview} onSign={onSign} />
          </td>
        </tr>
      )}
    </>
...
function ShipmentDetail({ shipment, onPreview, onSign }) {
  const isSigned = shipment.status === "已签收";
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{
        ...
        {[
          ...
          { k: "发货日期", v: shipment.date,           mono: true },
          { k: "预计到达", v: shipment.eta || "—",      mono: true },
        ].map((it, idx) => (
          <div key={idx}>
            <div className="dim" style={{ fontSize: 11, marginBottom: 4, letterSpacing: "0.04em" }}>{it.k}</div>
            <div style={{
      ...
      {shipment.items && shipment.items.length > 0 && (
        <div style={{
          ...
          <div style={{
            ...
            <span className="mono dim" style={{ fontSize: 10.5, letterSpacing: "0.1em" }}>
              本次发货明细 · {shipment.items.length} 项
            </span>
          </div>
              ...
              <tr>
                {["产品", "型号", "数量", "SN 段"].map((h, idx) => (
                  <th key={h} style={{
                    ...
                    padding: "6px 14px 10px", fontSize: 10, fontWeight: 500,
                    color: "var(--ink-4)", letterSpacing: "0.08em", textTransform: "uppercase",
                  }}>{h}</th>
                ))}
              </tr>
      ...
      {/* Attachments — express slip + receipt */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <AttachCard
          title="快递单"
          file={shipment.express}
          tone="info"
          onPreview={() => shipment.express && onPreview?.(shipment, "express")}
          emptyHint="发货时上传"
        />
        <AttachCard
          title="签收单"
          file={shipment.receipt}
          tone="success"
          onPreview={() => shipment.receipt && onPreview?.(shipment, "receipt")}
          emptyHint="签收时上传"
          emptyAction={!isSigned ? (
            <Btn variant="accent" size="sm" iconL="upload" onClick={() => onSign?.(shipment)}>
              签收入库
            </Btn>
          ) : null}
        />
...
function AttachCard({ title, file, tone, onPreview, emptyHint, emptyAction }) {
  const accent = {
    info:    { color: "var(--info)",    bg: "var(--info-soft)"    },
    success: { color: "var(--success)", bg: "var(--success-soft)" },
  }[tone] || { color: "var(--ink-3)", bg: "var(--bg-sunk)" };
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 8 }}>
        <span className="mono dim" style={{ fontSize: 10.5, letterSpacing: "0.1em", textTransform: "uppercase" }}>
          {title}
        </span>
        {!file && emptyHint && (
          <span className="dim" style={{ fontSize: 11 }}>{emptyHint}</span>
        )}
      </div>
      {file ? (
        <button onClick={onPreview}
          style={{
            ...
            background: accent.bg, border: `1px solid transparent`,
        ...
        >
          <span style={{
            color: accent.color, display: "inline-flex",
          }}>
        ...
        <div style={{
          ...
          <span className="dim" style={{ fontSize: 12, flex: 1 }}>暂未上传</span>
          {emptyAction}
        </div>
      )}
    </div>
