...
| 形式 | 连续横向滚动，条间留大段空白 | 见下「从淡入淡出改成滚动」 |
...
第一版是原地淡入淡出、6s 一换，理由写在这里：跑马灯读到一半会跑掉，手机端与无障碍都差。
上线一看，这条带子太安静 —— 淡入淡出没有"有新东西"的信号，眼睛压根不往那儿去。
它的职责就是引起注意，安静就是失职。

中间试过"一条走完下一条再进场"，同一时刻只有一条，读谁没有歧义 —— 但代价是大部分时间
带子上是空的，5 条一轮要 70 秒。最后定的是**整条连续滚动，靠留白把条目隔开**：每条前面
垫一段 `clamp(220px, 22vw, 420px)` 的空白（1135px 宽的视口上实测 264px）。宽屏上同时露出
两三条，但每条都自成一段，不会两个标题在同一行上争视线。

空白就是空白，不放分隔符号。中途试过在中点摆一个 ◆，看着像装饰噪音 —— 带子上只该有
通知本身。

速度 80 px/s，当前 5 条一轮 32 秒。

### 无缝循环怎么做的

把条目列表当作一"组"，按视口宽度复制若干份拼在轨道上，让轨道往左平移**正好一组宽**然后
从头循环。平移一组之后下一组恰好落在上一组原来的位置，看不出接缝。组数取
`ceil((视口宽 + 组宽) / 组宽)`，保证任何时刻视口都被填满。

副本是 JS 造的，对无障碍是纯噪音：标 `aria-hidden`、链接加 `tabindex="-1"`，只有真身那一组
可聚焦。悬停用**事件委托**绑在容器上 —— 逐个绑监听会漏掉副本。

组宽在 `document.fonts.ready` 之后重量一次：字体异步加载，首次量到的是 fallback 字体的宽度，
按它算出的循环距离会对不上。用 `fonts.ready` 而不是 `window.load` —— 后者还要等图片，
那时轮播早跑偏了。

## 配色：原来的底色根本不达标

原来是 `--accent` 50% 压在 `--bg-page` 上，实际渲染成 `#E0AF9B`，白字对比度 **1.95** ——
远低于 WCAG AA 的 4.5，用户的原话是"颜色不够鲜艳"。换成实色渐变
`#A8482A → #BE5730`，两端对白字是 5.79 / 4.57，都过 AA。

这两个值在明暗两套主题里都写死，不跟 `--accent` 走：横幅是一条自带底色的带子，不是随
页面深浅变化的表面；而暗色主题的 `--accent` 是 `#E08A6C`，偏亮，白字只有 2.62。

## 三个退化情形

- 只有 1 条 → 照样滚（职责是引起注意，静止就没意义了），复制若干份填满视口
- 一条都没有 → 整条不渲染
- `prefers-reduced-motion` → 不滚、也不复制副本，改成可以自己横向拖动的一条，信息一条不少

## 踩到的坑

**切片改写把文档接了三遍。** 用脚本替换 CSS 段时写的是 `i1 = s.index('</style>')` ——
找到的是文件最上面那个只放了一行 `material-symbols` 的 `<style>…</style>`，位置在横幅 CSS
**之前**，于是 `s[:i0] + NEW + s[i1:]` 把整个文档尾巴又接了一遍。连做三次就堆出三份横幅
CSS，最后一份在层叠里胜出 —— 表现是"改了没反应"，查了半天才发现服务端吐出的 HTML 里同一个
选择器有三份。结束位置必须从 `i0` 之后找（`s.index('</style>', i0)`），改完再自检每个关键
选择器只出现一次。

`.dash-banner-item` 是 `display:flex` 的块级元素，放进普通块容器会竖着堆，横幅高一度
229px —— 装它们的组容器也得是 flex。

圆点已随连续滚动去掉（滚动条带里"跳到第 3 条"没有意义），下面这条只作历史记录：
`at-theme.css:108` 有全局 `body.at-page button { background:none; padding:0 }`，
曾把圆点的背景和热区 padding 全压掉 —— 未选中的点直接不可见，样式必须带
