...
**但"绝对地址"和"指向别处"是两回事。** 第一版把所有带主机名的地址一律拒掉，于是从
浏览器地址栏整条复制 —— 最自然的那个动作 —— 会撞一句"请用 / 开头的站内路径"，而用户
复制的明明就是本系统的页面。错的只是写法，目的地本来就对，没有理由让人自己去删前缀。

现在按目的地分三档：

| 输入 | 处理 |
|---|---|
| 指向本站的绝对地址（比对 `request.host` 与 `X-Forwarded-Host`，回环的几种写法算同一个） | 削成路径存，不报错。`http://127.0.0.1:5096/wiki/at?q=1#s2` → `/wiki/at?q=1#s2` |
| 指向别处的 `https://` | 当外链原样存 |
| 指向别处的 `http://` | 拒绝，并把该填的路径直接写在报错里（多半是别台机器的内网地址，发给同事就是 404） |

`javascript:` / `data:` / `//evil.com` / 裸域名照旧拒绝。
