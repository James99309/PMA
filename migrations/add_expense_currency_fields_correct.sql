...
-- 明细货币索引  
CREATE INDEX IF NOT EXISTS idx_expense_details_currency 
ON expense_details(currency);

-- 明细的报销单ID和货币复合索引（用于汇总计算）
CREATE INDEX IF NOT EXISTS idx_expense_details_expense_currency 
ON expense_details(expense_id, currency);

-- 5. 添加字段注释（PostgreSQL语法）
COMMENT ON COLUMN expenses.currency IS '报销单主货币，用于最终结算和统计';
COMMENT ON COLUMN expense_details.currency IS '发票实际货币类型';
COMMENT ON COLUMN expense_details.invoice_amount IS '发票原始金额（发票货币）';
COMMENT ON COLUMN expense_details.current_amount IS '换算金额（报销单货币）';
COMMENT ON COLUMN expense_details.exchange_rate IS '换算汇率（发票货币→报销单货币）';