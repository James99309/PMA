...
const props = defineProps({
  ...
  // title/placeholder 为空时用 i18n 默认; 调用方可覆盖
  title:      { type: String, default: '' },
  placeholder:{ type: String, default: '' },
})
const titleText = computed(() => props.title || t('noteSheet.title'))
const placeholderText = computed(() => props.placeholder || t('noteSheet.placeholder'))
      ...
      <div v-if="modelValue" class="fixed inset-0 z-50">
        ...
        <div class="absolute left-0 right-0 rounded-t-3xl px-5 pt-4 pb-4"
          ...
          <p class="font-serif text-[17px] font-semibold mb-3"
            style="color: var(--color-ink);">{{ titleText }}</p>
          ...
          <div class="flex gap-2 mt-3">
            ...
            <button @click="onSubmit" :disabled="submitting || !text.trim()"
              ...
              {{ submitting ? t('noteSheet.submitting') : t('common.submit') }}
            </button>
          </div>
        </div>
      </div>
